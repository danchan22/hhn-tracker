  ...
  const houseRanksMap = useMemo(() => {
    ...
    const calcRankMap = (key: string, isDesc: boolean) => {
      const sorted = [...houseAnalyticsStats].sort((a, b) => {
        ...
        if (key === 'ratingOverall' || key === 'ratingScare' || key === 'ratingCool') {
          const avgA = getHouseAverages(a.name, allHouseRatings, selectedAttendeeFilter);
          const avgB = getHouseAverages(b.name, allHouseRatings, selectedAttendeeFilter);
          if (key === 'ratingOverall') {
            valA = avgA ? Number(avgA.overall) : 0;
            valB = avgB ? Number(avgB.overall) : 0;
          } else if (key === 'ratingScare') {
            valA = avgA ? Number(avgA.scare) : 0;
            valB = avgB ? Number(avgB.scare) : 0;
          } else if (key === 'ratingCool') {
            valA = avgA ? Number(avgA.cool) : 0;
            valB = avgB ? Number(avgB.cool) : 0;
          }
        }
    ...
    // Ratings, Total Visits, Total Wait: Higher value = #1
    ...
    // Avg Wait, Avg Posted, +/- Posted: Lower/Shortest value = #1
    calcRankMap('avgWait', false);
    calcRankMap('avgExpected', false);
    calcRankMap('diff', false);

    return ranks;
  }, [houseAnalyticsStats, allHouseRatings, selectedAttendeeFilter, getHouseAverages]);
        ...
        <div style={{ background: 'rgba(18, 18, 26, 0.85)', padding: '10px 12px', borderRadius: '14px', border: '1px solid #2A2A3C', marginBottom: '16px', backdropFilter: 'blur(8px)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '4px', marginBottom: analyticsSubTab === 'Houses' ? '6px' : '0px' }}>
            {[
              ...
              return (
                <button key={item.key} onClick={() => handleAnalyticsSortClick(item.key)} style={{ padding: '6px 2px', borderRadius: '8px', border: isActive ? '1px solid #DC2626' : '1px solid #2A2A3C', background: isActive ? '#DC2626' : '#1A1A26', color: isActive ? '#FFF' : '#A0AEC0', fontSize: '9.5px', fontWeight: '800', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                  {item.label}{arrow}
                </button>
              );
            })}
          ...
          {analyticsSubTab === 'Houses' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '4px' }}>
              {[
                ...
                const arrow = isActive ? (analyticsSortOrder === 'desc' ? ' ▼' : ' ▲') : '';
                return (
                  <button key={item.key} onClick={() => handleAnalyticsSortClick(item.key)} style={{ padding: '6px 2px', borderRadius: '8px', border: isActive ? '1px solid #FDA30C' : '1px solid #2A2A3C', background: isActive ? '#FDA30C' : '#1A1A26', color: isActive ? '#000' : '#FDA30C', fontSize: '10px', fontWeight: '800', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                    {item.label}{arrow}
                  </button>
                );
              })}
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '24px' }}>
            {houseAnalyticsStats.map(stat => {
              ...
              const ranks = houseRanksMap[stat.name] || {};
